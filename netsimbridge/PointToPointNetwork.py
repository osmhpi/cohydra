...
class PointToPointNetwork(object):
    ...
    def create(self):
        ...
        self.ptp_helper = ns.point_to_point.PointToPointHelper()
        # self.set_delay(self.delay)
        # self.set_data_rate(self.datarate)
        devices = self.ptp_helper.Install(node_container)  # , self.csma_channel)

        for i in range(0, len(self.system_nodes)):
            connected_node = self.system_nodes[i]
            connected_node.ptp_netdevice = devices.Get(i)
            connected_node.system_node.connect_to_netdevice(self.name, connected_node.ptp_netdevice,
